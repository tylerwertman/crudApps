...
const BrightIdeas = () => {
  ...
  const toastIdeaAdded = (idea, cookieName) =>
    toast.success(
      cookieName !== undefined
        ? `➕ You added an idea`
        : `➕ ${idea.addedByString} added an idea`
    );
  const toastIdeaDeleted = (idea, cookieName) =>
    toast.error(
      cookieName !== undefined
        ? `🗑 You deleted an idea`
        : `🗑 ${idea.addedByString} deleted an idea`
    );
  const toastIdeaFav = (idea, cookieName) =>
    toast.success(
      cookieName !== undefined
        ? `👍 You favorited an idea`
        : `👍 A user favorited an idea`
    );
  const toastIdeaUnfav = (idea, cookieName) =>
    toast.error(
      cookieName !== undefined
        ? `👎 You unfavorited an idea`
        : `👎 A user unfavorited an idea`
    );
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        const sortedIdeas = res.data.idea.sort(
          (a, b) => b.favoritedBy.length - a.favoritedBy.length
        );
  ...
  useEffect(() => {
    ...
    const handleIdeaFavorited = (updatedIdea) => {
      setIdeaList((ideaList) => {
        ...
        const socketSortedList = updatedList.sort(
          (a, b) => b?.favoritedBy?.length - a?.favoritedBy?.length
        );
        return socketSortedList;
      });
    ...
    const handleIdeaDeleted = (deletedIdea) => {
      setIdeaList((ideaList) =>
        ideaList.filter((idea) => idea._id !== deletedIdea._id)
      );
  ...
  const handleAddIdeaSubmit = (e) => {
    e.preventDefault();
    axios
      .post("http://localhost:8000/api/ideas", oneIdea, {
  ...
  const favoriteIdea = (idea) => {
    axios
      .post(
        `http://localhost:8000/api/ideas/${idea._id}/favorite`,
        {},
        { withCredentials: true }
      )
      .then((res) => {
  ...
  const unfavoriteIdea = (idea) => {
    axios
      .post(
        `http://localhost:8000/api/ideas/${idea._id}/unfavorite`,
        {},
        { withCredentials: true }
      )
      .then((res) => {
  ...
  const removeIdea = (idea) => {
    axios
      .delete(`http://localhost:8000/api/ideas/${idea._id}`)
      .then((res) => {
        toastIdeaDeleted(idea, user.name);
  ...
  const handleSearchSubmit = (e) => {
    ...
    axios
      ...
      .then((res) => {
        const searchedResults = res.data.idea.filter((idea) =>
          idea.idea.toLowerCase().includes(searchQuery.toLowerCase())
        );
        setIdeaList(searchedResults);
        const sortedSearchedIdeas = searchedResults.sort(
          (a, b) => b.favoritedBy.length - a.favoritedBy.length
        );
  ...
  const returnToAllIdeas = () => {
    ...
    setSearch(!search);
    axios
      .get("http://localhost:8000/api/ideas", { params: { search: "" } })
      .then((res) => {
        const sortedIdeas = res.data.idea.sort(
          (a, b) => b.favoritedBy.length - a.favoritedBy.length
        );
  ...
  return (
    <div className="mt-5">
      ...
      <div className={mode === "dark" ? "mainDivDark" : "mainDivLight"}>
        <div
          className={
            mode === "dark" ? "col-sm-8 mx-auto text-light" : "col-sm-8 mx-auto"
          }
        >
          {/* Add Idea Form */}
          <form
            className={
              mode === "dark" ? "mx-auto text-light mt-5" : "mx-auto mt-5"
            }
            onSubmit={handleAddIdeaSubmit}
          >
            {oneIdea.idea && oneIdea.idea?.length < 2 ? (
              <p className="text-danger">Idea must be at least 2 characters</p>
            ) : null}
            {errors.idea ? (
              <p className="text-danger">{errors.idea.message}</p>
            ) : null}
            {windowWidth > 575 ? (
              <div className="input-group col-10">
                <div className="form-floating">
                  <input
                    ...
                    name="idea"
                    value={oneIdea.idea}
                    onChange={handleAddIdeaChange}
                    placeholder="Add a new idea!"
                  />
                  <label className="darkText" htmlFor="idea">
                    Add a new idea!
                  </label>
                </div>
                <button
              ...
              <div>
                ...
                <button
                  type="submit"
                  className="input-group-text btn btn-success mt-3 col-10"
                  onSubmit={handleAddIdeaSubmit}
                >
                  Add idea!
                </button>
        ...
        <h3 className="mt-3" onClick={returnToAllIdeas}>
          {search ? "Search Ideas" : "All Ideas 🔍"}
        </h3>
        {/* SEARCH */}
        {search ? (
            ...
            return (
              <div className="mt-5" key={idea._id}>
                {idea?.addedBy ? (
                  <>
                    ...
                    <img
                      ...
                      src={`${idea.addedBy?.profilePicture}`}
                      alt=""
                      style={{ width: "40px", height: "40px", padding: "3px" }}
                    />{" "}
                    <Link to={`/users/${idea.addedBy._id}`}>
                      @{idea?.addedBy.displayName}
                    </Link>
                    <span> said:</span>&nbsp;
                  </>
                ) : (
                  <span>
                    On{" "}
                    {new Date(idea.createdAt).toLocaleString("en-US", options)}{" "}
                    at{" "}
                    {new Date(idea.createdAt).toLocaleString([], {
                      timeStyle: "short",
                    })}
                ...
                <span className="text-end">
                  {currentItems[index]?.favoritedBy?.length ===
                  0 ? null : currentItems[index]?.favoritedBy?.length === 1 ? (
                    <>
                      <span>Liked by </span>
                ...
                {
                  // fav/unfav
                  currentItems[index]?.favoritedBy?.some(
                    (ideaObj) => ideaObj._id === user?._id
                  ) && mode === "dark" ? (
                    <>
                      <button
                        className="btn btn-outline-danger"
                        onClick={() => unfavoriteIdea(idea)}
                      >
                        ✩
                      </button>
                      &nbsp;
