...
const UserDetail = (props) => {
  ...
  return (
    <div className={mode === "dark" ? "mainDivDark mt-5" : "mainDivLight mt-5"}>
      <br />
      <br />
      <div
        className={"popup mx-auto"}
        style={{
          display: showDeletePopup ? "block" : "none",
          color: "white",
          top: "25%",
        }}
      >
      ...
      <h2>
        ...
        />{" "}
        {oneUser?.name} (@{oneUser?.displayName})
      </h2>
      <h6>Joined on: {new Date(oneUser?.createdAt).toLocaleString()}</h6>
      <h6>Last updated: {new Date(oneUser?.updatedAt).toLocaleString()}</h6>
      <br />
      <table className={mode === "dark" ? "mx-auto tableDark" : "mx-auto"}>
        <thead>
          <tr className={mode === "dark" ? "flex tableDark" : "flex"}>
            {allTabs.map((tabs, i) => (
              <th
                className={
                  currentTab === i && mode === "dark"
                    ? "tab clicked tableDark"
                    : currentTab === i
                    ? "tab clicked"
                    : "tab"
                }
        ...
        {currentTab === 0 ? (
          <div className="col-md-6 mx-auto">
            ...
            {oneUser?.booksAdded?.map((usersAddedBooks) => {
              return (
                <h6 key={usersAddedBooks._id}>
                  <Link to={`/books/${usersAddedBooks?._id}`}>
                    {usersAddedBooks?.title}
                  </Link>
                </h6>
              );
            })}
          ...
          <div className="col-md-6 mx-auto">
            ...
            {oneUser?.booksFavorited?.map((usersFavBooks) => {
              return (
                <h6 key={usersFavBooks._id}>
                  <Link to={`/books/${usersFavBooks?._id}`}>
                    {usersFavBooks?.title}
                  </Link>
                </h6>
              );
          ...
          <div className="col-md-6 mx-auto">
            ...
            {oneUser?.ideasAdded?.map((usersAddedIdeas) => {
              return (
                <h6 key={usersAddedIdeas._id}>
                  <Link to={`/ideas/${usersAddedIdeas?._id}`}>
                    {usersAddedIdeas?.idea}
                  </Link>
                </h6>
              );
            })}
          ...
          <div className="col-md-6 mx-auto">
            ...
            {oneUser?.ideasFavorited?.map((usersFavIdeas) => {
              return (
                <h6 key={usersFavIdeas._id}>
                  <Link to={`/ideas/${usersFavIdeas?._id}`}>
                    {usersFavIdeas?.idea}
                  </Link>
                </h6>
              );
      ...
      {id === user._id ? (
        <button
          className={mode === "dark" ? "btn btn-danger" : "btn btn-dark"}
